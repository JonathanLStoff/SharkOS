...
void bootRadioTest() {
  ...
  // ── CC1101 #1 and #2: already tested during deviceSetup (SPI state guaranteed correct there) ──
  Serial.printf("[bootRadioTest] CC1101 #1: %s\n", radioOk_cc1101_1 ? "OK" : "FAIL");
  Serial.printf("[bootRadioTest] CC1101 #2: %s\n", radioOk_cc1101_2 ? "OK" : "FAIL");
  ...
  // ── NFC (PN532): already probed in deviceSetup while SPI bus was in correct state ──
  Serial.printf("[bootRadioTest] NFC (PN532): %s\n", radioOk_nfc ? "OK" : "not detected");
  ...
  // ── IR: initialized in deviceSetup via IrReceiver.begin/IrSender.begin ──
  Serial.printf("[bootRadioTest] IR TX=GPIO%d RX=GPIO%d: %s\n",
    irsenderpin, irrecivepin, radioOk_ir ? "OK" : "not initialized");

  // ── Summary ──
  bool requiredFailed = !radioOk_cc1101_1 || !radioOk_cc1101_2;

  Serial.printf("[bootRadioTest] Summary: CC1101_1=%s CC1101_2=%s LoRa=%s(opt) NFC=%s(opt) nRF24=%s(opt) IR=%s(opt)\n",
    radioOk_cc1101_1 ? "OK" : "FAIL",
    radioOk_cc1101_2 ? "OK" : "FAIL",
    radioOk_lora     ? "OK" : "N/C",
    radioOk_nfc      ? "OK" : "N/C",
...
String getRadioStatusJson() {
  ...
  radios["ir"]       = radioOk_ir       ? "ok" : "not_connected";
