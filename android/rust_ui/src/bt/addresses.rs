...
fn build_oui_map() -> HashMap<u32, &'static str> {
    ...
    for line in OUI_RAW.lines() {
        ...
        // split at first whitespace (handles tab or spaces)
        if let Some(i) = line.find(|c: char| c.is_whitespace()) {
            let (k, v) = line.split_at(i);
            let vendor = v.trim();
            if let Ok(prefix) = u32::from_str_radix(k, 16) {
...
/// Lookup vendor by MAC address (accepts formats like `aa:bb:cc:dd:ee:ff`, `aabb.ccdd.eeff`, `aabbccddeeff`, etc.).
/// Returns `Some(&'static str)` vendor name on match, or `None` if unknown / invalid.
pub fn vendor_for_mac(mac: &str) -> Option<&'static str> {
    // normalize: keep only hex digits
    let hex: String = mac.chars().filter(|c| c.is_ascii_hexdigit()).collect();
    if hex.len() < 6 { return None; }
    let prefix_hex = &hex[..6];
    let prefix = u32::from_str_radix(prefix_hex, 16).ok()?;
    OUI_MAP.get_or_init(build_oui_map).get(&prefix).copied()
}
...
mod tests {
    ...
    #[test]
    fn basic_vendor_lookups() {
        assert_eq!(vendor_for_mac("00:00:00"), Some("Officially Xerox"));
        assert_eq!(vendor_for_mac("fc:ff:aa:11:22:33"), Some("IEEE Registration Authority"));
        assert_eq!(vendor_for_mac("00-00-01-12-34-56"), Some("SuperLAN-2U"));
        assert_eq!(vendor_for_mac("aa:bb:cc"), None);
    }
}